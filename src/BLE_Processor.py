...
class BLE(object):
    def get_epsg(self, gdb_path):
        ...
        if unique_count == 1:
            ...
            print(
                "Warning: Rasters spatial references do not have corresponding EPSG codes."
            )
        elif unique_count > 1:
            print(
                "Warning! There are multiple EPSG codes for the rasters in the provided gdb file"
            )
    ...
    def river_centerline_shp(self, geo_file_path, ReachInfoDF):
                ...
                for linenumer, line in enumerate(TXTFile):
                    if linenumer >= reachstart and linenumer < reachend:
                        if line.startswith("River Reach"):
                            ...
                            ThisXs = flopy.utils.Util2d.load_txt(
                                (Count, 2), TXTFile, np.float64, "(10F16.0)"
                            )
                            geometry = [
                                Point(xy) for xy in zip(ThisXs[:, 0], ThisXs[:, 1])
                            ]

                            ThisReach_Points = gpd.GeoDataFrame(geometry=geometry)
                            ThisReach_Points["river_reach"] = "%s,%s" % (
                                rivername,
                                reachname,
                            )
        ...
        AllReachesLines = AllReachesPoints.groupby(["river_reach"], as_index=False)[
            "geometry"
        ].apply(lambda x: LineString(x.tolist()))
        AllReachesLines["river"] = AllReachesLines.apply(
            lambda row: row.river_reach.split(",")[0], axis=1
        )
        AllReachesLines["reach"] = AllReachesLines.apply(
            lambda row: row.river_reach.split(",")[1], axis=1
        )
    ...
    def xsec_shp(self, geo_file_path, ReachInfoDF):
                ...
                for linenumer, line in enumerate(TXTFile):
                    if linenumer >= reachstart and linenumer < reachend:
                        if line.startswith("Type RM Length L Ch R"):
                            ...
                            ThisXsectionPoints["Xsectionid"] = "%s,%s" % (
                                reachname,
                                stn,
                            )
        ...
        AllXsLines = AllXsPoints.groupby(["Xsectionid"], as_index=False)[
            "geometry"
        ].apply(lambda x: LineString(x.tolist()))
        AllXsLines["river"] = AllXsLines.apply(
            lambda row: row.Xsectionid.split(",")[0], axis=1
        )
        AllXsLines["reach"] = AllXsLines.apply(
            lambda row: row.Xsectionid.split(",")[1], axis=1
        )
        AllXsLines["stn"] = AllXsLines.apply(
            lambda row: row.Xsectionid.split(",")[2], axis=1
        )
        ...
        with open(geo_file_path) as TXTFile:
            for lineid, line in enumerate(TXTFile):
                if line.startswith("River Reach"):
                    ...
                    continue
        ReachInfoDF = pd.DataFrame(
            ReachInfo, columns=["id", "river, reach", "StartlineNo"]
        )
